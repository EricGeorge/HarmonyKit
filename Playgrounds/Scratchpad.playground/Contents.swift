...
//print("The submedian chord for \(scale.root) \(scale.type) is \(scale.submediantTriad): \(scale.submediantTriad.notes) which is the \(scale.type.roman[ScaleDegree.submediant.rawValue]) chord")
//print("The leading chord for \(scale.root) \(scale.type) is \(scale.subtonicTriad): \(scale.subtonicTriad.notes) which is the \(scale.type.roman[ScaleDegree.subtonic.rawValue]) chord")
